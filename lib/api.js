...
`;
  const data = await fetchAPI(gql, { variables: { lang: API_LOCALE } });
  return data?.allPosts?.edges;
}

export async function getAllPostsForHome(previewData, { locale }) {
  ...
  `,
    {
      previewData,
      variables: {
        lang: localesMatching[locale] || localesMatching.es,
      },
    }
  );
...
export async function getPostAndMorePosts(slug, previewData, { locale }) {
  ...
  query PostBySlug($slug: String!, $lang: String!) {
   ...
   morePosts: allPosts(sortBy: date_DESC, first: 3, lang: $lang) {
      edges {
        node {
          title
          content
